...
def _render_jade(app):	
	...
	for path, subdirs, filenames in os.walk(jade_dir):
		for jade_file in filenames:
			jade_name = os.path.splitext(jade_file)[0]
			html_file     = os.path.join(dest, jade_name+".html")
	def action(source, dest):
		dest = os.path.dirname(dest)
		cmd = 'pyjade'
		subprocess.call([cmd, '-c' , 'jinja' , '-o', os.path.normcase(html_file), source], shell=False)
...
def _collect_static(app):
	...
	dir_to_collect = ['css', 'images', 'js', 'videos', 'data']
